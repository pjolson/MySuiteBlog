...
`search.mjs` recognizes distinctive message families before falling back to the MiniSearch package already installed with VitePress. Rules are classified where they are declared: `rule()` marks a definitive family whose hits select the intended candidates directly, while `broad()` marks wording that occurs on import types beyond the documented entries — a broad hit boosts its candidates alongside lexical and inferred-context matches instead of replacing them, and never claims a confirmed match. A `clarifications` table lists whole-query fragments (such as a bare `already received`) that ask the visitor for more detail instead of naming a restriction; complete messages are unaffected because the patterns are anchored. Full-message matching tolerates variable values without stripping meaningful limits such as 61 or 100%. Natural-language matching allows modest spelling errors, but identifier-shaped tokens stay exact: they are harvested from message fragments and aliases (underscores or digits mark them), an entry can declare additional ones in an optional `protectedTokens` array, and a short documented prefix list covers bare field IDs that carry no such marker. Unknown messages do not get a fabricated answer. The original input is never normalized in place.
